...
def heatmap_with_annotations(adata, val, transpose=True, obs_keys = None, cmap='viridis', 
    ...
    if obs_keys is not None:
        ...
        for col in obs_keys:
            ...
            if pd.api.types.is_numeric_dtype(data_col):
                # For numeric data, map values to colors using the colormap
                norm = mcolors.Normalize(vmin=data_col.min(), vmax=data_col.max())
                use_colors[col] = [col_cmap(norm(val)) for val in data_col]
            else:
                # For categorical data, map categories to colors using the palette
    ...
    if add_color_legend and legend_data:
        for labels, palette, title in legend_data:
            add_legend(g.ax_heatmap, labels, palette, title=title, bbox_anchor=(1, 1))
